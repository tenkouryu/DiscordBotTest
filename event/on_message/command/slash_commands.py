...
from . import channel, chat, member, scenario, server
...
def register_slash_commands(tree: app_commands.CommandTree[discord.Client]) -> None:
    ...
    member = app_commands.Group(name="member", description="メンバー操作")
    member_role = app_commands.Group(name="role", description="メンバーのロール操作", parent=member)
    server = app_commands.Group(name="server", description="サーバー操作")
    server_role = app_commands.Group(name="role", description="サーバーのロール操作", parent=server)
    server_member = app_commands.Group(name="member", description="サーバーメンバー操作", parent=server)
    channel = app_commands.Group(name="channel", description="チャンネル操作")
    chat = app_commands.Group(name="chat", description="添付ファイル操作")
    scenario = app_commands.Group(name="scenario", description="シナリオ操作")
    ...
    async def member_role_add_command(interaction: discord.Interaction, member: discord.Member, role_name: str) -> None:
        await _run(interaction, member.role_add.main, f"/member_role_add {member.mention} {role_name}", mentions=[member])
    ...
    async def member_role_get_command(interaction: discord.Interaction, member_name: str) -> None:
        await _run(interaction, member.role_get.main, f"/member_role_get {member_name}")
    ...
    async def member_role_remove_command(interaction: discord.Interaction, member: discord.Member, role_name: str) -> None:
        await _run(interaction, member.role_remove.main, f"/member_role_remove {member.mention} {role_name}", mentions=[member])
    ...
    async def member_role_set_command(interaction: discord.Interaction, file: discord.Attachment) -> None:
        await _run(interaction, member.role_set.main, "/member_role_set", attachments=[file])
    ...
    async def member_role_template_command(interaction: discord.Interaction) -> None:
        await _run(interaction, member.role_template.main, "/member_role_template")
    ...
    async def server_role_add_command(interaction: discord.Interaction, role_name: str) -> None:
        await _run(interaction, server.role_add.main, f"/server_role_add {role_name}")
    ...
    async def server_role_edit_command(interaction: discord.Interaction, role_name: str, setting: str, value: str) -> None:
        await _run(interaction, server.role_edit.main, f"/server_role_edit {role_name} {setting} {value}")
    ...
    async def server_role_get_command(interaction: discord.Interaction) -> None:
        await _run(interaction, server.role_get.main, "/server_role_get")
    ...
    async def server_role_set_command(interaction: discord.Interaction, file: discord.Attachment) -> None:
        await _run(interaction, server.role_set.main, "/server_role_set", attachments=[file])
    ...
    async def server_role_template_command(interaction: discord.Interaction) -> None:
        await _run(interaction, server.role_template.main, "/server_role_template")
    ...
    async def server_role_remove_command(interaction: discord.Interaction, role_name: str) -> None:
        await _run(interaction, server.role_remove.main, f"/server_role_remove {role_name}")
    ...
    async def server_member_list_command(interaction: discord.Interaction) -> None:
        await _run(interaction, lambda message: server.member_list.main(interaction.client, message), "/server_member_list")

    @channel.command(name="create", description="チャンネルを作成")
    async def channel_create_command(interaction: discord.Interaction, channel_type: str, channel_name: str, category_name: str | None = None) -> None:
        await _run(interaction, channel.create.main, f"/channel_create {channel_type} {channel_name} {category_name or ''}")

    @channel.command(name="get", description="チャンネル一覧をCSVで取得")
    async def channel_get_command(interaction: discord.Interaction) -> None:
        await _run(interaction, channel.get.main, "/channel_get")

    @channel.command(name="move", description="チャンネルをカテゴリーへ移動")
    async def channel_move_command(interaction: discord.Interaction, channel: discord.TextChannel, category_name: str) -> None:
        await _run(interaction, channel.move.main, f"/channel_move {channel.mention} {category_name}", channel_mentions=[channel])

    @channel.command(name="set", description="CSVからチャンネルを設定")
    async def channel_set_command(interaction: discord.Interaction, file: discord.Attachment) -> None:
        await _run(interaction, channel.set.main, "/channel_set", attachments=[file])

    @channel.command(name="template", description="チャンネルCSVテンプレート")
    async def channel_template_command(interaction: discord.Interaction) -> None:
        await _run(interaction, channel.template.main, "/channel_template")

    @chat.command(name="get", description="添付ファイルを取得")
    async def chat_get_command(interaction: discord.Interaction, channel: discord.TextChannel, start_date: str | None = None, extension: str | None = None) -> None:
        ...
        await _run(interaction, chat.get.main, f"/chat_get {arguments}", channel_mentions=[channel])

    @chat.command(name="template", description="添付ファイル取得CSVテンプレート")
    async def chat_template_command(interaction: discord.Interaction) -> None:
        await _run(interaction, chat.template.main, "/chat_template")

    @scenario.command(name="template", description="シナリオCSVテンプレート")
    async def scenario_template_command(interaction: discord.Interaction) -> None:
        await _run(interaction, scenario.template.main, "/scenario_template")

    @scenario.command(name="set", description="CSVからシナリオを登録")
    async def scenario_set_command(interaction: discord.Interaction, file: discord.Attachment) -> None:
        await _run(interaction, scenario.set.main, "/scenario_set", attachments=[file])

    @scenario.command(name="list", description="登録済みシナリオ一覧")
    async def scenario_list_command(interaction: discord.Interaction) -> None:
        await _run(interaction, scenario.list.main, "/scenario_list")

    @scenario.command(name="start", description="シナリオを開始")
    async def scenario_start_command(interaction: discord.Interaction, scenario_id: str, start_step: int | None = None) -> None:
        ...
        await _run(interaction, scenario.start.main, content)

    @scenario.command(name="delete", description="シナリオを削除")
    async def scenario_delete_command(interaction: discord.Interaction, scenario_id: str) -> None:
        await _run(interaction, scenario.delete.main, f"/scenario_delete {scenario_id}")

    @scenario.command(name="export", description="シナリオをCSVで出力")
    async def scenario_export_command(interaction: discord.Interaction) -> None:
        await _run(interaction, scenario.export.main, "/scenario_export")

    tree.add_command(member)
    tree.add_command(server)
    tree.add_command(channel)
    tree.add_command(chat)
    tree.add_command(scenario)
